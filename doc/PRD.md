...
**Status dokumen:** Sudah diaudit; implementasi berjalan. **Fase 0 sampai 10 selesai** — fondasi & rule engine, data dev, Dashboard Utama, Direktori & Profil Talenta, Peta Talenta & Perbandingan Kandidat, Master Data & Kualitas Data, Rule Engine (Jabatan Target), Talent Pool & Workflow Nominasi, Auth & RBAC, Laporan & Ekspor CSV, API Eksternal `/api/v1`, serta Kategori & Validasi Riwayat. Halaman yang sudah dibangun ditandai ✅ di §6; lihat [`../phase.md`](../phase.md) untuk urutan fase, keputusan teknis, dan usulan perbaikan halaman (§8 U-1…U-12).

> **Yang masih menunggu keputusan pemilik proses** tercatat di §10 (12 keputusan terbuka) dan di `CLAUDE.md`. Dua yang paling berdampak dan datang dari `doc/doc_tambahan_2/sample(1).md`: versi rubrik **Substansi Riwayat Jabatan** (2 kategori vs 5 kategori Plt/Plh) dan **sumber data Penilaian Kinerja** (`ekinerja.pu.go.id` vs `karir.pu.go.id/enom`). Keduanya mengubah angka atau integrasi, jadi default yang berjalan sengaja tidak diubah tanpa jawaban.
...
| **Kategori Riwayat Diklat (master)** ✅ | Kamus kategori pelatihan berjenjang — rumpun **Manajerial** (PIM II/III/IV) & **Teknis** (PBJ, Hukum Kontrak, Manajemen Konstruksi), diturunkan dari `doc/doc_tambahan_2/sample(1).md` lembar 6 + PP 11/2017. Tiap kategori punya `setara_jenjang` (untuk memeriksa syarat pelatihan jabatan target) dan daftar kata kunci **pengusul**. Skemanya `doc/sql/014` (9 kategori terisi). Rumpun tanpa turunan ditandai TIDAK BISA DIPAKAI, karena rumpun tidak boleh dipilih saat memetakan diklat — memetakan ke "Pelatihan Teknis" tanpa menyebut teknis apa membuat syarat pelatihan tidak bisa diperiksa. Tidak ada tombol hapus, hanya nonaktifkan: kategori yang sudah dipakai adalah dasar skor yang harus tetap bisa dipertanggungjawabkan | Tambah/ubah kategori & kata kunci pengusul, aktif/nonaktifkan | Super Admin, Admin Talenta |
| **Validasi Riwayat Pegawai** ✅ | Antrian dua bagian: **(a)** 182 nama diklat berbeda dari seluruh pegawai, masing-masing perlu satu keputusan kategori — beserta usulan dari kata kunci dan **alasan** usulannya; **(b)** riwayat jabatan yang jenis penugasannya (Definitif/Plt/Plh) belum dipastikan. Satu keputusan per **nama diklat**, bukan per pegawai: di data dev itu 182 keputusan alih-alih 264, dan di produksi selisihnya jauh lebih besar. Ditutup dengan tanda "riwayat pegawai ini sudah saya periksa" (`pegawai.riwayat_divalidasi_*`). Skemanya `doc/sql/014` | Setujui/ubah/tolak usulan kategori, tetapkan jenis penugasan, tandai riwayat pegawai sudah diperiksa | **Pengelola Unit** (data unitnya sendiri — §3: *"Input/validasi riwayat data pegawai unitnya"*), Admin Talenta, Super Admin |
...
| Halaman | Isi / Konten | Aksi Kunci | Role |
|---|---|---|---|
| **Manajemen Klien & Token API** ✅ *(dua halaman PRD disatukan)* | Daftar klien (nama, kode, kontak, no. MoU/PKS, status, scope) **beserta token masing-masing** dan **pemakaian 7 hari terakhir**. Angka pemakaian ikut ditampilkan karena tanpa itu halaman ini hanya mendaftar siapa yang *boleh*, bukan siapa yang *memakai* — dan kredensial yang menganggur setahun layak dicabut. Klien AKTIF tanpa nomor MoU ditandai bahaya, begitu juga kolom scope yang bentuknya tidak dikenali (diperlakukan tanpa izin — kalau tidak ditandai, "tidak bisa apa-apa" terbaca sebagai kebijakan yang disengaja). **Disatukan dari dua halaman PRD**: token tidak punya arti tanpa kliennya, dan memisahnya membuat setiap pemeriksaan menuntut berpindah halaman lalu mencocokkan sendiri | Tambah/ubah klien & scope, terbitkan token, cabut token. **Klien tidak bisa AKTIF tanpa nomor MoU/PKS** (ditegakkan di server), dan token hanya diterbitkan untuk klien AKTIF. Token **ditampilkan tepat sekali** dan tidak pernah masuk `audit_log` — token yang bisa dibaca ulang dari jejak audit tidak pernah benar-benar jadi rahasia klien. Cabut **tidak menghapus** barisnya: `api_activity_log` menunjuk ke sana | Super Admin |
| **Log Aktivitas API** ✅ | Riwayat pemanggilan per klien & token (endpoint, waktu, response code, waktu respons, IP), berpaginasi & tersaring di SQL. **`ditolak` (4xx) dipisah tegas dari `galat` (5xx)**: 4xx berarti klien yang salah — token mati, scope tidak mencakup, rate limit — yaitu sistem bekerja benar; 5xx berarti kita yang salah. Menggabungkannya jadi satu angka "error" membuat lonjakan penolakan yang wajar terlihat seperti kerusakan, dan kerusakan yang sesungguhnya tersamar di antaranya. **Batas yang ditulis di halaman:** permintaan dengan token yang sama sekali tidak dikenali TIDAK muncul di sini — `api_activity_log.api_client_id` NOT NULL, jadi tidak ada klien untuk diatribusikan; percobaan seperti itu hanya sampai ke log server | Filter hasil/endpoint/rentang tanggal | Super Admin |
| **Dokumentasi API** ✅ | Autentikasi Bearer (bentuk header ketat, contoh `curl`), daftar endpoint beserta scope & parameternya, aturan penyamaran data personal, tabel bentuk galat, dan batas rate limit. Daftar scope-nya **mengacu ke konstanta yang sama** dengan yang ditegakkan gerbang, jadi dokumentasi tidak bisa menyebut scope yang tidak ada. **Internal, bukan portal publik** — dan halamannya menjelaskan kenapa: kredensial developer yang terpisah dari sesi internal berarti permukaan autentikasi *ketiga* (di samping sesi & Bearer) beserta pendaftaran, pemulihan sandi, dan pencabutannya sendiri. Sampai itu diputuskan, isinya dikirim ke klien lewat kanal yang sudah ada di MoU/PKS | — (referensi) | Super Admin, Admin Talenta |
...
12. ⚙️ **Definisi "sesuai dengan jabatan target".** Indikator Kesesuaian Bidang Ilmu (§B.2.2) dan Pengembangan Kompetensi (§B.2.3) bergantung pada frasa itu, tapi tidak ada tempat menyimpan *apa* yang dianggap sesuai — akibatnya keduanya tidak bisa dihitung otomatis. **Default terpasang, dan keduanya kini BERBEDA:**
    - **Bidang ilmu** → tetap `jabatan_target.kata_kunci_relevansi` (JSON), diedit dari Editor Jabatan Target (§6.5). `"semua"` berarti semua bidang ilmu diperbolehkan. Perlu konfirmasi daftar kata kunci per jabatan target dari pemilik proses.
    - **Diklat** → sejak `doc/sql/014`–`015` **tidak lagi dari kata kunci**, melainkan dari kategori yang divalidasi manusia (`pemetaan_diklat`) dibandingkan dengan `jabatan_target_syarat_diklat`. Sebabnya terukur: dari 182 nama diklat nyata di `pupr_dev` hanya 36 cocok dengan pola kategori mana pun. Aturan lama "`semua` tidak membebaskan syarat diklat" **tidak berlaku lagi** — ia tambalan atas satu kolom yang dipakai dua maksud sekaligus.
    - Konsekuensi yang perlu diketahui pemilik proses: indikator diklat & Substansi Riwayat Jabatan sekarang bisa bernilai **"tidak diketahui"** dan ditandai `perlu_review`, bukan diberi angka. Selama antrian validasi belum dikerjakan, skor total turun rata-rata **3,92 poin** (terukur, maks −6,33) — sementara **kelayakan kandidat tidak berubah sama sekali**.

Dua berikutnya muncul dari **paket dokumen tambahan** (Blueprint Arsitektur, FSD, SRD, DRD, Implementation Plan — v1.0 Juli 2026, lihat [`doc_tambahan/`](doc_tambahan/) dan disposisinya di [`doc_tambahan/DISPOSISI.md`](doc_tambahan/DISPOSISI.md)). Keduanya **mengubah siapa boleh apa**, jadi tidak diputuskan sendiri oleh implementasi:

13. ⚙️ **Daftar aktor berbeda dari yang terpasang.** FSD §2 memakai **6 aktor** — Super Admin, Admin Talenta, **Admin Data**, **Pejabat Reviewer**, Pimpinan, Auditor/Viewer — dan **tidak memuat Pengelola Unit**. Padahal Pengelola Unit adalah dasar seluruh pembatasan data per unit yang sudah berjalan (`lib/lingkup.ts`, ditegakkan di SQL, diuji smoke Fase 7). **Default terpasang:** 5 peran seperti §3, dengan *Admin Data* & *Pejabat Reviewer* dibaca sebagai penamaan lain dari Admin Talenta & Pimpinan. Perlu konfirmasi: apakah keduanya memang peran **baru** yang perlu ditambahkan, dan kalau ya, siapa yang mewarisi pembatasan per unit?
14. ⚙️ **Kandidat yang gagal syarat: masuk ranking atau tidak.** Paket tambahan **bertentangan dengan dirinya sendiri**: Blueprint §3 menulis "eligibility dipisahkan dari match score agar kandidat tidak langsung diranking sebelum lolos syarat wajib", sedangkan diagram Alur Matching §7 di dokumen yang sama menggambar jalur `FAIL` **tetap** menuju Ranking & Gap Analysis. **Default terpasang: masuk** — sesuai diagram §7 dan §6.5 di PRD ini ("termasuk yang tidak lolos syarat, karena skornya tetap berguna sebagai pembanding"). Perlu didamaikan di dokumen sumbernya lebih dulu; mengubahnya menyentuh halaman Kandidat yang sudah diuji.
