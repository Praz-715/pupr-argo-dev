...
3. ✅ **Satu deklarasi syarat jabatan** *(bidang ilmu; syarat diklat menyusul)*. Data lembar 6 kini di tiga tempat: `jabatan_target_persyaratan` (9 baris) · `jabatan_target_syarat_diklat` (8) · `jabatan_target.kata_kunci_relevansi` (3). Lembar 6 sendiri **satu baris per jabatan**, jadi memecahnya ke tiga tempat membuat penambahan di satu tempat diam-diam mengubah yang lain. Deklarasinya disatukan; **gerbang dan rubrik dua-duanya membaca dari sana.** Yang **tidak** dilebur: fungsinya — lihat catatan.
...
- [x] Empat jabatan kosong tanpa target **tetap tampil** dengan tombol "Jadikan draft", dan jumlahnya disebut — bukan disembunyikan karena tidak bisa dihitung. *Terbukti: jumlah tombol (4) dicocokkan ke `COUNT(*)` langsung dari DB, bukan ke angka yang ditulis halaman itu sendiri*
- [x] Tombol itu tidak menambah kolom apa pun: hasilnya `jabatan_target` DRAFT + satu `jabatan_target_anggota`. *Terbukti end-to-end: klik → editor terbuka → DB memuat `status='DRAFT'` + 1 anggota → draft **tidak** ditawarkan di pemilih Peta Talenta (hanya AKTIF) → dihapus, anggotanya ikut lewat cascade*
- [x] Setelah konsolidasi, `grep` menemukan **satu** tempat menulis bidang ilmu. *Terbukti: `grep -E "UPDATE|INSERT" kata_kunci_relevansi` → satu `UPDATE` yang menulis nilai (di `simpanPersyaratan`, jalur BIDANG_ILMU), satu yang mengosongkan saat syaratnya dihapus, dan dua `INSERT` yang melahirkan `JSON_ARRAY()` kosong. Form profil & `ubahJabatanTarget` **tidak lagi menyentuhnya***
- [x] Syarat **diklat** juga dideklarasikan dari tab yang sama. *Sebelumnya satu-satunya bagian lembar 6 **tanpa permukaan UI** — hanya bisa diubah lewat SQL, padahal ia menentukan indikator Pengembangan Kompetensi 5% untuk seluruh kandidat. Terbukti: 6 kotak centang = 6 kategori turunan aktif, 3 rumpun jadi **judul kelompok** bukan pilihan, dan satu simpan menulis 1 baris (`wajib=1`, bukan rumpun) lalu dipulihkan*
...
- [x] Kasus regresi bernama: **Yuliana Wijaya tetap skor 90,88 dengan `eligible = 0`** (*"Eselon tertinggi NON_ESELON di bawah syarat minimal III"*). *Dipasang sebagai dua pemeriksaan permanen di `verifikasi-data` (46 → 48): satu agregat menuntut **ketiga** jabatan target punya inversi, satu kasus bernama supaya kegagalannya bisa ditelusuri ke satu baris*
- [x] Berkas smoke `fase-11` (**27 pemeriksaan**); `ukur:kueri` memuat kueri Kotak 9 per target dan lolos ambangnya (**77 kueri · 327 ms**, tiga kueri baru 3 ms masing-masing)

> **Rute yang pindah tempat: pengalihannya di `next.config`, bukan di halaman — dan itu dipelajari dengan cara yang keras.** Percobaan pertama memakai halaman yang memanggil `permanentRedirect()`. Hasilnya bukan galat, melainkan **skeleton yang tidak pernah selesai**: pengalihannya tersangkut di batas `<Suspense>`, halaman menjawab HTTP 200, dan URL tidak bergerak. Itu lebih buruk daripada 404, karena tidak menyatakan apa pun dan penggunanya menunggu sesuatu yang tidak akan datang — kelas cacat yang sama dengan "cek email Anda" pada Lupa Password. `redirects()` di `next.config.ts` diselesaikan **sebelum** render dimulai, jadi tidak ada yang bisa tersangkut. Query string terbawa sendiri; fragment `#jabatan-kosong` tidak bisa dijanjikan server karena ia milik peramban.

> **Peran nav kedua halaman diperiksa DULU, bukan diasumsikan.** Keduanya `Super Admin · Admin Talenta · Pimpinan` — identik, jadi peleburan tidak menggeser siapa boleh melihat apa. Kalau berbeda, peleburan ini akan menjadi perubahan wewenang yang menyamar sebagai perapian tata letak. Entri nav "Jabatan Kosong & Risiko" tetap ada tapi `luarSidebar`: masih ditemukan lewat command palette dengan kata yang benar-benar dipakai orang ("bup", "pensiun", "lowong"), tanpa menambah entri sidebar kedua yang tujuannya sama.

> **Bidang ilmu ternyata SUDAH menyimpang, dan itu tidak dibetulkan otomatis.** Kedua penyimpanan di `pupr_dev` berbeda isi: target 3 menyimpan `teknik · sipil · konstruksi` di gerbang tapi **lima** kata kunci di rubrik (`+ kompetensi · produktivitas`), dan target 1–2 masih menyimpan kata **diklat** (`ppbj`, `kepemimpinan`) di daftar bidang ilmu — sisa sebelum `doc/sql/015` memindahkan syarat diklat ke tabelnya sendiri. Menyeragamkannya otomatis berarti membuang kata kunci yang **sekarang masih menaikkan skor** sebagian kandidat, yaitu mengubah skor orang tanpa ada yang memutuskannya. Jadi tab Persyaratan **menampilkan** selisihnya, menunjukkan isi kedua versi, dan menyatakan bahwa menyimpan akan menyeragamkan **dan menggeser skor** — dengan saran menjalankan Simulasi & Diff lebih dulu. Keputusannya milik manusia.

> **Satu jalur tulis yang harus ikut ditutup, bukan cuma satu yang ditambah.** Menyatukan tulisnya di `simpanPersyaratan()` tidak ada artinya selama form profil jabatan target **juga** bisa menulis `kata_kunci_relevansi`. Field itu dicabut dari form profil dan dari `ubahJabatanTarget()`; pembagiannya sekarang bersih — form profil mengurus **identitas** (kode, nama, deskripsi), tab Persyaratan mengurus **syarat**. Ikut terbetulkan: panel peringatan di form itu masih mengklaim `"semua"` tidak membebaskan syarat diklat — aturan yang sudah **dibatalkan** oleh `015` dan sudah tercatat batal di PRD §10.12, tapi teksnya tertinggal di UI.

> **Syarat pelatihan: menambah kategori MELONGGARKAN syarat, bukan mengetatkannya — dan itu ditulis di panelnya.** Indikator Pengembangan Kompetensi hanya punya dua kategori skor (punya → 100, tidak → 50), jadi yang dinilai adalah "punya **minimal satu** yang cocok". Menambah kategori keempat memperbesar peluang cocok. Itu kebalikan dari dugaan wajar, dan tidak bisa ditebak dari kontrolnya. Sejalan dengan itu, kolom `wajib` **tidak** diberi kontrol di UI: ia ada di skema tapi belum dipakai perhitungan (ERD §3.2), dan kontrol tanpa akibat lebih buruk daripada kontrol yang tidak ada. Panel juga menyatakan bahwa **kosong ≠ gagal** — Ka Balai BJKW memang tidak ada di lembar 6, jadi indikatornya *tidak diketahui* + `perlu_review`, dan panel itu tidak boleh terlihat seperti kekeliruan yang harus diisi buru-buru dengan karangan.

> **`labelPending` melahirkan varian baru jebakan #1, dan ia bahkan tidak butuh kata yang sudah ada di layar.** Tombol simpan memakai `labelPending="Menyimpan…"`, jadi teksnya **berubah** selama aksinya terbang. Uji yang menunggu teks lama *lenyap* (`waitFor state: 'detached'` pada lokator ber-`hasText`) karena itu lolos **seketika** — bukan karena pekerjaannya selesai, tapi karena indikator pending-nya sendiri yang mengubahnya. Akibatnya pembacaan DB sesudahnya mendahului `INSERT`, uji melaporkan "tersimpan 0 baris", dan barisnya justru muncul beberapa ratus milidetik kemudian — meninggalkan data uji yang membuat jalan berikutnya gagal karena sebab yang berbeda lagi. Obatnya: `tungguDb()`, menunggu **keadaan DB** alih-alih bentuk UI. Aturan umumnya: kalau tombolnya punya pending state, jangan pernah memakai teksnya sebagai penanda selesai.

> **Penolakan rumpun ditegakkan di server tapi BELUM terbukti lewat HTTP.** UI tidak menawarkan rumpun sama sekali, jadi satu-satunya cara mencapai cabang itu adalah memanggil aksinya langsung dengan id rumpun — dan itu menuntut sesi. Ia dipertahankan sebagai pertahanan berlapis (halaman bisa basi; gerbang tidak boleh), tapi statusnya sama dengan `unitWajib` pada ekspor: **ada, teruji lewat pembacaan kode, belum lewat permintaan nyata.**

> **Satu bug lama ikut terpancing dan dibetulkan.** `PetaSkeleton` mengunci daftar skeleton-nya dengan **nama kelas Tailwind** sebagai `key`, dan lebarnya berulang (`w-40` & `w-44` masing-masing dua kali) → React memperingatkan kunci ganda dan boleh menghilangkan salah satunya. Tidak pernah terlihat sebelumnya karena skeleton hanya tampil sekejap; baru muncul setelah smoke Fase 11 menavigasi halaman itu belasan kali.
...
> **Kenapa gerbang tidak boleh dilebur ke rubrik, walau deklarasinya disatukan.** `S2` sebagai **gerbang** berarti "tidak lolos syarat"; `S2` sebagai **rubrik** berarti "Magister → 90 poin". Input sama, keluaran beda. Yang memikul bebannya adalah **inversi**, dan itu ada di data dev: Yuliana Wijaya berskor **90,88** dengan `eligible = 0` (eselon tertinggi NON_ESELON, di bawah syarat minimal III) — di atas banyak kandidat yang lolos, walau **bukan** skor tertinggi (Budi Santoso 91,85 lebih tinggi dan lolos). Kalau syarat dilebur jadi indikator rubrik, ketidaklolosan akan menekan skornya sendiri dan inversi seperti itu **tidak mungkin lagi ada** — jadi hilangnya inversi adalah tanda paling awal bahwa pemisahannya runtuh. Dijaga dua pemeriksaan baru di `verifikasi-data`: satu agregat (ketiga jabatan target harus punya inversi) dan satu kasus bernama. Pemisahan ini juga yang dipertahankan kedua dokumen sumber: BLUEPRINT §5 menaruh "Seleksi Kelayakan" **sebelum** "Hitung Match Score", dan `sample(1).md` sendiri memisahkan lembar 6 dari lembar 2/3/4.
